...
import 'package:vip/Game/Lv2/test2.dart'; // 타이머를 위한 라이브러리
...
class _WaitPageState extends State<WaitPage1> with TickerProviderStateMixin {
    ...
    try {
      ...
      if (currentUser != null) {
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => Test2Page(userId: userId), // userId 전달
          ),
        );
      } else {
        // 로그인되지 않은 경우
  ...
  void _startCountdown() {
    Timer.periodic(const Duration(seconds: 1), (timer) {
      if (countdown == 1) {
        timer.cancel(); // 타이머 종료
        _navigateToTest2(); // Test1Page로 이동
      } else {
        setState(() {
          countdown--;
        });
          ...
          Align(
            ...
            child: Padding(
              ...
              child: ElevatedButton(
                onPressed: () {
                  _navigateToTest2(); // 버튼 클릭 시 Test1Page로 이동
                },
