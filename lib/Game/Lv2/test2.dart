...
class Test2Page extends StatefulWidget {
  ...
  final String userId;
  const Test2Page({super.key, required this.userId, this.onNextStepUnlocked});
...
/*  FlutterSoundRecorder? _audioRecorder;
  ...
  void _startListening() async {
    ...
    if (available) {
      ...
      await _speechToText.listen(
        onResult: (result) {
          setState(() {
            responseController.text = result.recognizedWords; // 실시간으로 음성 인식 텍스트 업데이트
          });
        },
        listenFor: Duration(seconds: 60), // 60초 동안 음성을 인식하도록 설정
        onSoundLevelChange: (level) {
          // 추가적으로 소리 레벨이 변경될 때마다 처리할 수 있는 부분
        },
      );
    } else {
  ...
  Future<void> _uploadToFirebase(File file) async {
    try {
      ...
      final title = 'Test2에서 녹화된 영상 (${formattedDate} ${timestamp
  ...
  Future<void> fetchInterviewQuestion() async {
    ...
    try {
      final question = await openAIService.generateInterviewQuestion2();
      if (mounted) {
        setState(() {
